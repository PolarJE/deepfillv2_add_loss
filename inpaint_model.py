...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(
            self, FLAGS, batch_data, training=True, summary=True,
            reuse=False):
        if FLAGS.guided:
        ...
        batch_predicted = x2 # refinement result
        losses = {}
        ...
        #---------------------------perceptual_loss, style_loss-----------------------#
        out_im = batch_predicted 
        gt_resize = tf.image.resize_images(batch_data / 127.5 - 1, [256, 256])
        image_pre = vgg.preprocess(gt_resize)
        fai_imgt = {}
        net = vgg.net(FLAGS.vgg_path, image_pre)
        for layer in FLAGS.vgg_layer:
            fai_imgt[layer] = net[layer]

        image_pre = vgg.preprocess(tf.image.resize_images(batch_pos, [256, 256]))
        fai_imout = {}
        net = vgg.net(FLAGS.vgg_path, image_pre)
        for layer in FLAGS.vgg_layer:
            fai_imout[layer] = net[layer]

        Im_compt = batch_complete
        im_compt = tf.image.resize_images(Im_compt, [256, 256])
        ...
        per_sty_loss = get_total_loss(out_im, batch_pos, fai_imout, 
                                    fai_imgt, fai_compt, FLAGS.vgg_layer, im_compt)
        losses['per_sty_loss'] = per_sty_loss
        #---------------------------perceptual_loss, style_loss-----------------------#
        ...
        if FLAGS.ae_loss:
            ...
            losses['g_loss'] += losses['per_sty_loss']
